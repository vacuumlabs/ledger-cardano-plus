...
extension _SignTransactionV7 on CardanoSignTransactionOperation {
  Future<SignedTransactionData> _invokeV7(LedgerSendFct send) async {
  ...
  Future<void> _signTxInit(LedgerSendFct send, List<LedgerSigningPath> witnessPaths) async {
    final data = SerializationV7.serializeV7TxInit(
  ...
  Future<void> _signTxSetTtl(BigInt ttl, LedgerSendFct send) async {
    await send(
      LedgerSimpleOperation(
        ...
        p1: p1StageTtl,
        p2: p2Unused,
        data: SerializationV7.serializeV7TxTtl(ttl),
        prependDataLength: true,
  ...
  Future<void> _signTxAddCertificate(
    ...
    CardanoVersion cardanoVersion,
    LedgerSendFct send,
  ) async {
    final Uint8List data = SerializationV7.serializeV7TxCertificate(certificate, cardanoVersion);
    ...
    switch (certificate) {
      case StakePoolRegistration():
        if (VersionCompatibility.checkVersionCompatibility(cardanoVersion).supportsPoolRegistrationAsOperator) {
          await _signTxAddStakePoolRegistrationCertificate(certificate, send);
        } else {
          await _signTxAddStakePoolRegistrationCertificateLegacy(certificate, send);
        }
      default:
    }
  }
  ...
  Future<void> _signTxAddStakePoolRegistrationCertificateLegacy(
    ...
    await send(
      LedgerSimpleOperation(
        ...
        data: SerializationV7.serializeV7PoolInitialParamsLegacy(certificate.pool),
        prependDataLength: true,
    ...
    for (final owner in certificate.pool.owners) {
      await send(
        LedgerSimpleOperation(
          ...
          p1: p1StageCertificates,
          p2: p2OwnersLegacy,
          data: SerializationV7.serializeV7PoolOwner(owner),
          prependDataLength: true,
    ...
    await send(
      LedgerSimpleOperation(
        ...
        p1: p1StageCertificates,
        p2: p2MetadataLegacy,
        data: SerializationV7.serializeV7PoolMetadata(certificate.pool.metadata),
        prependDataLength: true,
  ...
  Future<void> _signTxAddStakePoolRegistrationCertificate(
    ...
    await send(
      LedgerSimpleOperation(
        ...
        p1: p1StageCertificates,
        p2: p2InitPool,
        data: SerializationV7.serializeV7PoolInitialParams(poolParams.pool),
        prependDataLength: true,
    ...
    await send(
      LedgerSimpleOperation(
        ...
        p1: p1StageCertificates,
        p2: p2PoolKey,
        data: SerializationV7.serializeV7PoolKey(poolParams.pool.poolKey),
        prependDataLength: true,
    ...
    await send(
      LedgerSimpleOperation(
        ...
        p1: p1StageCertificates,
        p2: p2Financials,
        data: SerializationV7.serializeV7Financials(poolParams.pool),
        prependDataLength: true,
    ...
    await send(
      LedgerSimpleOperation(
        ...
        p1: p1StageCertificates,
        p2: p2RewardAccount,
        data: SerializationV7.serializeV7PoolRewardAccount(poolParams.pool.rewardAccount),
        prependDataLength: true,
    ...
    await send(
      LedgerSimpleOperation(
        ...
        p1: p1StageCertificates,
        p2: p2Metadata,
        data: SerializationV7.serializeV7PoolMetadata(poolParams.pool.metadata),
        prependDataLength: true,
        debugName: "Sign Transaction Stake Pool Registration Metadata",
      ),
    );
  ...
  Future<TxAuxiliaryDataSupplement?> _signTxSetAuxiliaryData(
    ...
    final serializedAuxData = SerializationV7.serializeV7TxAuxiliaryData(auxiliaryData);
    await send(
      LedgerSimpleOperation(
        cla: claCardano,
        ins: InstructionType.signTransaction.insValue,
    ...
    final Future<TxAuxiliaryDataSupplement?>? Function() invoker = switch (auxiliaryData) {
      CIP36Registration() => () async {
        ...
        if (VersionCompatibility.checkVersionCompatibility(version).supportsCIP36) {
          final serializedInitData = SerializationV7.serializeV7CVoteRegistrationInit(params);
          await send(
            LedgerSimpleOperation(
              cla: claCardano,
              ins: InstructionType.signTransaction.insValue,
        ...
        if (params.votePublicKey != null || params.votePublicKeyPath != null) {
          final serializedVoteKeyData = SerializationV7.serializeV7CVoteRegistrationVoteKey(
        ...
        if (params.delegations != null) {
          for (final delegation in params.delegations!) {
            final serializedDelegationData = SerializationV7.serializeV7CVoteRegistrationDelegation(delegation);
            await send(
              LedgerSimpleOperation(
                cla: claCardano,
                ins: InstructionType.signTransaction.insValue,
        ...
        final serializedStakingKeyData = SerializationV7.serializeV7CVoteRegistrationStakingPath(params.stakingPath);
        await send(
          LedgerSimpleOperation(
            cla: claCardano,
            ins: InstructionType.signTransaction.insValue,
        ...
        final serializedPaymentAddressData = SerializationV7.serializeV7CVoteRegistrationPaymentDestination(
        ...
        final serializedNonceData = SerializationV7.serializeV7CVoteRegistrationNonce(params.nonce);
        await send(
          LedgerSimpleOperation(
            cla: claCardano,
            ins: InstructionType.signTransaction.insValue,
        ...
        if (VersionCompatibility.checkVersionCompatibility(version).supportsCIP36) {
          final serializedVotingPurposeData = SerializationV7.serializeV7CVoteRegistrationVotingPurpose(
  ...
  Future<TxAuxiliaryDataSupplement?> _signTxSetAuxiliaryDatabBeforev2_3(
    ...
    if (auxiliaryData is! ArbitraryHash) {
      throw LedgerCardanoValidationException("Auxiliary data type not implemented");
    }
  ...
  Future<void> _signTxSetValidityIntervalStart(BigInt validityIntervalStart, LedgerSendFct send) async {
    final Uint8List data = SerializationV7.serializeV7TxValidityStart(validityIntervalStart);
  ...
  Future<void> _signTxSetMint(
    ...
    await send(
      LedgerSimpleOperation(
        ...
        p1: p1StageMint,
        p2: p2MintBasicData,
        data: SerializationV7.serializeV7MintBasicParams(mint),
        prependDataLength: true,
  ...
  Future<void> _signTxAddTokenBundle(
    ...
    for (final assetGroup in tokenBundle) {
      final Uint8List assetGroupData = SerializationV7.serializeV7AssetGroup(assetGroup);
      await send(
        LedgerSimpleOperation(
          cla: claCardano,
          ins: InstructionType.signTransaction.insValue,
      ...
      for (final token in assetGroup.tokens) {
        final Uint8List tokenData = SerializationV7.serializeV7Token(token);
        await send(
          LedgerSimpleOperation(
            cla: claCardano,
            ins: InstructionType.signTransaction.insValue,
  ...
  Future<void> _signTxAddCollateralOutput(
    ...
    await send(
      LedgerSimpleOperation(
        ...
        data: SerializationV7.serializeV7TxOutputBasicParams(collateralOutput, version, network),
        prependDataLength: true,
  ...
  Future<void> _signTxAddVoterVotes(ParsedVoterVotes voterVotes, LedgerSendFct send) async {
    await send(
      LedgerSimpleOperation(
        ...
        p2: p2Unused,
        data: SerializationV7.serializeV7VoterVotes(voterVotes),
        prependDataLength: true,
  ...
  Future<void> _signTxAddOutput(
    ...
    await send(
      LedgerSimpleOperation(
        ...
        data: SerializationV7.serializeV7TxOutputBasicParams(output, version, network),
        prependDataLength: true,
    ...
    if (outputDatum != null) {
      await send(
        LedgerSimpleOperation(
          ...
          p1: p1StageOutputs,
          p2: p2OutputDatum,
          data: SerializationV7.serializeV7TxOutputDatum(outputDatum, version),
          prependDataLength: true,
    ...
    if (outputReferenceScriptHash != null) {
      await send(
        LedgerSimpleOperation(
          ...
          p1: p1StageOutputs,
          p2: p2OutputScript,
          data: SerializationV7.serializeV7TxOutputRefScript(outputReferenceScriptHash),
          prependDataLength: true,
