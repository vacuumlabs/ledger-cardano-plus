...
class CardanoSignTransactionOperation extends LedgerComplexOperation<SignedTransactionData> {
  ...
  Future<void> _signTxAddCertificate(
    ...
    final Uint8List data = SerializationUtils.serializeTxCertificate(certificate, cardanoVersion);
  ...
  Future<TxAuxiliaryDataSupplement?> _signTxSetAuxiliaryData(
    ...
    final Future<TxAuxiliaryDataSupplement?>? Function() invoker = switch (auxiliaryData) {
      CIP36Registration() => () async {
        ...
        if (VersionCompatibility.checkVersionCompatibility(version).supportsCIP36) {
          final serializedVotingPurposeData = SerializationUtils.serializeCVoteRegistrationVotingPurpose(
  ...
  Future<SignedTransactionData> _invokeV8(LedgerSendFct send) async {
    ...
    final rawTxBody = SerializationUtils.serializeV8RawTxBody(
      tx: signingRequest.tx,
      version: cardanoVersion,
      network: network,
    );
    ...
    await send(
      LedgerSimpleOperation(
        ...
        p1: p1V8SignTxInit,
        p2: p2Unused,
        data: SerializationUtils.serializeV8TxInit(
          tx: signingRequest.tx,
          signingMode: signingRequest.signingMode,
          numWitnesses: witnessPaths.length,
          options: signingRequest.options,
  ...
  Future<TxAuxiliaryDataSupplement> _signTxV8AuxData(
    ...
    final initData = SerializationUtils.serializeV8CVoteAuxDataInit(
      params: params,
      version: cardanoVersion,
      network: network,
    );
    ...
    for (int i = 0; i < delegations.length; i++) {
      ...
      final delegationData = SerializationUtils.serializeV8CVoteAuxDataDelegation(delegations[i]);
